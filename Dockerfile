...
#bitcoin core v0.2.3
WORKDIR /
RUN wget https://github.com/mocacinno/bitcoin_core_history/archive/refs/heads/v0.2.3.zip && \
    unzip v0.2.3.zip
WORKDIR /bitcoin_core_history-0.2.3
RUN mkdir -p obj/nogui && \
    ...
    strip bitcoin
RUN ln -s /bitcoin_core_history-0.2.3/bitcoin /usr/local/bin
...
USER bitcoinuser
LABEL org.opencontainers.image.revision="manual-trigger-20251105"
