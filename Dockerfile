...
# -------------------------------
# Build Bitcoin Core v0.2.1
# -------------------------------
WORKDIR /src
RUN curl -fsSL https://github.com/mocacinno/bitcoin_core_history/archive/refs/heads/v0.2.1.zip -o bitcoin.zip && \
    echo "ccda4b5d8761ad99f2867947fcac7819ae6c76a87879a6c1aa2b14af9c31f745 bitcoin.zip" | sha256sum -c - && \
    unzip bitcoin.zip && \
    cd bitcoin_core_history-0.2.1 && \
    mkdir -p obj/nogui && \
    ...
    strip bitcoin && \
    ln -s /src/bitcoin_core_history-0.2.1/bitcoin /usr/local/bin
