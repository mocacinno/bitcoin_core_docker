...
WORKDIR /
RUN wget https://github.com/mocacinno/bitcoin_core_history/archive/refs/heads/v0.3.11.zip
RUN unzip v0.3.11.zip
WORKDIR /bitcoin_core_history-0.3.11
#run g++ -v -c util.cpp
...
COPY --from=builder /bitcoin_core_history-0.3.11/bitcoind /usr/local/bin
