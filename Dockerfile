...
# -------------------------------
# Build Bitcoin Core v0.2.4
# -------------------------------
WORKDIR /src
RUN curl -fsSL https://github.com/mocacinno/bitcoin_core_history/archive/refs/heads/v0.2.4.zip -o bitcoin.zip && \
    echo "3b2f21931e0cc05d29beb4db8900015250b2d7d89af90601e0605dd22ba20270 bitcoin.zip" | sha256sum -c - && \
    unzip bitcoin.zip && \
    cd bitcoin_core_history-0.2.4 && \
    mkdir -p obj/nogui && \
    ...
    strip bitcoin && \
    ln -s /src/bitcoin_core_history-0.2.4/bitcoin /usr/local/bin
