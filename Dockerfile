...
RUN git checkout tags/v0.12.1 -b v0.12.1 #v0.12.1
...
RUN ./autogen.sh #v0.12.1
RUN ldconfig
RUN ln -s /boost_1_63_0/stage/lib/libboost_system.so.1.63.0 /usr/lib64
RUN ./configure  --enable-util-cli --enable-util-tx --enable-util-wallet --enable-util-util #v0.12.1
RUN make -j "$(($(nproc) + 1))" #v0.12.1
WORKDIR /bitcoin/src
