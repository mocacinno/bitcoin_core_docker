...
LABEL org.opencontainers.image.version="v0.2.6"
...
# -------------------------------
# Build Bitcoin Core v0.2.6
# -------------------------------
WORKDIR /src
RUN curl -fsSL https://github.com/mocacinno/bitcoin_core_history/archive/refs/heads/v0.2.6.zip -o bitcoin.zip && \
    echo "1c3b2970450cc52c5cc10f18729a123126c844b6947f5fd72ebbc6a3978a058d bitcoin.zip" | sha256sum -c - && \
    unzip bitcoin.zip && \
    cd bitcoin_core_history-0.2.6 && \
    mkdir -p obj/nogui && \
    ...
    strip bitcoin && \
    ln -s /src/bitcoin_core_history-0.2.6/bitcoin /usr/local/bin
