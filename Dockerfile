...
#bitcoin v24.0
WORKDIR /
RUN wget https://github.com/bitcoin/bitcoin/archive/refs/tags/v24.0.zip && \
    unzip v24.0.zip
WORKDIR /bitcoin-24.0
RUN ./autogen.sh
...
RUN make -j "$(($(nproc) + 1))" 
WORKDIR /bitcoin-24.0/src
RUN strip bitcoin-util && strip bitcoind && strip bitcoin-cli && strip bitcoin-tx  


FROM registry.suse.com/bci/bci-minimal:15.7
COPY --from=builder /bitcoin-24.0/src/bitcoin-util /usr/local/bin
COPY --from=builder /bitcoin-24.0/src/bitcoin-cli /usr/local/bin
COPY --from=builder /bitcoin-24.0/src/bitcoin-tx /usr/local/bin
COPY --from=builder /bitcoin-24.0/src/bitcoind /usr/local/bin
