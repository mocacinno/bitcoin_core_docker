...
#bitcoin v0.11.0
WORKDIR /
RUN wget https://github.com/bitcoin/bitcoin/archive/refs/tags/v0.11.0.zip && \
    unzip v0.11.0.zip
WORKDIR /bitcoin-0.11.0
RUN ./autogen.sh #v0.11.0
RUN ldconfig
...
RUN make -j "$(($(nproc) + 1))" 
WORKDIR /bitcoin-0.11.0/src
RUN strip bitcoind && strip bitcoin-cli && strip bitcoin-tx


FROM registry.suse.com/bci/bci-minimal:15.6
COPY --from=builder /bitcoin-0.11.0/src/bitcoin-cli /usr/local/bin
COPY --from=builder /bitcoin-0.11.0/src/bitcoin-tx /usr/local/bin
COPY --from=builder /bitcoin-0.11.0/src/bitcoind /usr/local/bin
