...
RUN git checkout tags/v0.14.3 -b v0.14.3 #v0.14.3
RUN zypper ref -s && zypper --non-interactive install libopenssl-devel
RUN ./autogen.sh #v0.14.3
RUN ldconfig
RUN ln -s /boost_1_63_0/stage/lib/libboost_system.so.1.63.0 /usr/lib64
RUN ./configure  --enable-util-cli --enable-util-tx --enable-util-wallet --enable-util-util #v0.14.3
RUN make -j "$(($(nproc) + 1))" #v0.14.3
WORKDIR /bitcoin/src
