...
RUN zypper ref -s && zypper --non-interactive install git wget libevent-devel awk libdb-4_8-devel sqlite3-devel libleveldb1 clang7 gcc-c++ python3-devel python3 && zypper --non-interactive install -t pattern devel_basis #prereqs

RUN zypper addrepo https://download.opensuse.org/repositories/devel:/gcc/SLE-15/devel:gcc.repo
RUN zypper --gpg-auto-import-keys ref -s #gcc4.3
RUN zypper --non-interactive install gcc43 gcc43-c++ #gcc4.3
ENV CC=gcc-4.3
ENV CXX=g++-4.3


RUN wget https://sourceforge.net/projects/boost/files/boost/1.55.0/boost_1_55_0.tar.gz/download -O boost_1_55_0.tar.gz #boost1.55.0
RUN tar -xvf boost_1_55_0.tar.gz #boost1.55.0
ENV BOOST_ROOT=/boost_1_55_0
WORKDIR /boost_1_55_0
RUN ./bootstrap.sh
#RUN ./b2 -sBOOST_FILESYSTEM_VERSION=2 -j"$(($(nproc) + 1))" --toolset=gcc-4.3
RUN ./b2 -sBOOST_FILESYSTEM_VERSION=2 -j"$(($(nproc) + 1))"
#RUN ./b2 --with-thread
RUN ./b2 headers
...
RUN git checkout tags/v0.10.5 -b v0.10.5 #v0.10.5
...
RUN ldconfig
RUN ./autogen.sh #v0.10.5

RUN ./configure  --enable-util-cli --enable-util-tx --enable-util-wallet --enable-util-util --without-boost-thread --without-boost-chrono  #v0.10.5
RUN make -j "$(($(nproc) + 1))" #v0.10.5
WORKDIR /bitcoin/src
