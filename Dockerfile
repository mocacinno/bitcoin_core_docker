...
# -------------------------------
# Build Bitcoin Core v0.2.3
# -------------------------------
WORKDIR /src
RUN curl -fsSL https://github.com/mocacinno/bitcoin_core_history/archive/refs/heads/v0.2.3.zip -o bitcoin.zip && \
    echo "dfff2e6bea7ee8d459c9b49193e30193b147087a5bb376a846f920f4bccfbe21 bitcoin.zip" | sha256sum -c - && \
    unzip bitcoin.zip && \
    cd bitcoin_core_history-0.2.3 && \
    mkdir -p obj/nogui && \
    ...
    strip bitcoin && \
    ln -s /src/bitcoin_core_history-0.2.3/bitcoin /usr/local/bin
