...
#bitcoin v0.3.2
WORKDIR /
RUN wget https://github.com/bitcoin/bitcoin/archive/refs/tags/v0.3.2.zip && \
    unzip v0.3.2.zip
WORKDIR /bitcoin-0.3.2
RUN find /bitcoin-0.3.2/ -type f -exec dos2unix {} + && \
    mkdir -p obj/nogui && \
...
COPY --from=builder /bitcoin-0.3.2/bitcoind /usr/local/bin
