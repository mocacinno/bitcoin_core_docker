...
#bitcoin v25.2
WORKDIR /
RUN wget https://github.com/bitcoin/bitcoin/archive/refs/tags/v25.2.zip && \
    unzip v25.2.zip
WORKDIR /bitcoin-25.2
RUN ./autogen.sh 
...
RUN make -j "$(($(nproc) + 1))" 
WORKDIR /bitcoin-25.2/src
RUN strip bitcoin-util && strip bitcoind && strip bitcoin-cli && strip bitcoin-tx  

FROM registry.suse.com/bci/bci-minimal:15.7
COPY --from=builder /bitcoin-25.2/src/bitcoin-util /usr/local/bin
COPY --from=builder /bitcoin-25.2/src/bitcoin-cli /usr/local/bin
COPY --from=builder /bitcoin-25.2/src/bitcoin-tx /usr/local/bin
COPY --from=builder /bitcoin-25.2/src/bitcoind /usr/local/bin
