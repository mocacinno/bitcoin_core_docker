...
RUN make -j "$(($(nproc) + 1))" 
WORKDIR /bitcoin-26.1/src
RUN strip bitcoin-util && strip bitcoind && strip bitcoin-cli && strip bitcoin-tx  

FROM registry.suse.com/bci/bci-minimal:15.7
COPY --from=builder /bitcoin-26.1/src/bitcoin-util /usr/local/bin
COPY --from=builder /bitcoin-26.1/src/bitcoin-cli /usr/local/bin
COPY --from=builder /bitcoin-26.1/src/bitcoin-tx /usr/local/bin
COPY --from=builder /bitcoin-26.1/src/bitcoind /usr/local/bin
