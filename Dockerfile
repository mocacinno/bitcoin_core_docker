...
#bitcoin core v0.3.5
WORKDIR /
RUN wget https://github.com/mocacinno/bitcoin_core_history/archive/refs/heads/v0.3.5.zip
RUN unzip v0.3.5.zip
WORKDIR /bitcoin_core_history-0.3.5
...
COPY --from=builder /bitcoin_core_history-0.3.5/bitcoind /usr/local/bin
