...
#bitcoin v0.6.2
WORKDIR /
RUN wget https://github.com/bitcoin/bitcoin/archive/refs/tags/v0.6.2.zip && \
    unzip v0.6.2.zip
WORKDIR /bitcoin-0.6.2/src
COPY patch_mocacinno_net /bitcoin-0.6.2/src/patch_mocacinno_net
COPY patch_mocacinno_strlcpy /bitcoin-0.6.2/src/patch_mocacinno_strlcpy
...
COPY --from=builder /bitcoin-0.6.2/src/bitcoind /usr/local/bin
