...
#bitcoin v0.8.4
WORKDIR /
RUN wget https://github.com/bitcoin/bitcoin/archive/refs/tags/v0.8.4.zip && \
    unzip v0.8.4.zip
WORKDIR /bitcoin-0.8.4/src
COPY patch_mocacinno_net /bitcoin-0.8.4/src/patch_mocacinno_net
...
COPY --from=builder /bitcoin-0.8.4/src/bitcoind /usr/local/bin
