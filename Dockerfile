...
#bitcoin core v0.3.9
WORKDIR /
RUN wget https://github.com/mocacinno/bitcoin_core_history/archive/refs/heads/v0.3.9.zip
RUN unzip v0.3.9.zip
WORKDIR /bitcoin_core_history-0.3.9
#run g++ -v -c util.cpp
...
COPY --from=builder /bitcoin_core_history-0.3.9/bitcoind /usr/local/bin
