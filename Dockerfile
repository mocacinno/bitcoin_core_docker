...
#bitcoin v0.8.2
WORKDIR /
RUN wget https://github.com/bitcoin/bitcoin/archive/refs/tags/v0.8.2.zip && \
    unzip v0.8.2.zip
WORKDIR /bitcoin-0.8.2/src
COPY patch_mocacinno_net /bitcoin-0.8.2/src/patch_mocacinno_net
...
COPY --from=builder /bitcoin-0.8.2/src/bitcoind /usr/local/bin
...
USER bitcoinuser
LABEL org.opencontainers.image.revision="manual-trigger-20251110"
