...
WORKDIR /
RUN wget https://github.com/mocacinno/bitcoin_core_history/archive/refs/heads/v0.3.1.zip && \
    unzip v0.3.1.zip
WORKDIR /bitcoin_core_history-0.3.1
RUN zypper --non-interactive install dos2unix && \
    find /bitcoin_core_history-0.3.1/ -type f -exec dos2unix {} + && \
    mkdir -p obj/nogui && \
...
COPY --from=builder /bitcoin_core_history-0.3.1/bitcoind /usr/local/bin
