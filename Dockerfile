...
#bitcoin v0.5.1
WORKDIR /
RUN wget https://github.com/bitcoin/bitcoin/archive/refs/tags/v0.5.1.zip && \
    unzip v0.5.1.zip
WORKDIR /bitcoin-0.5.1/src
COPY patch_mocacinno_net /bitcoin-0.5.1/src/patch_mocacinno_net
COPY patch_mocacinno_strlcpy /bitcoin-0.5.1/src/patch_mocacinno_strlcpy
...
COPY --from=builder /bitcoin-0.5.1/src/bitcoind /usr/local/bin
