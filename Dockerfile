...
WORKDIR /
RUN wget https://github.com/bitcoin/bitcoin/archive/refs/tags/v0.3.24.zip
RUN unzip v0.3.24.zip
WORKDIR /bitcoin-0.3.24/src
RUN make -j"$(($(nproc) + 1))" -f makefile.unix bitcoind CXXFLAGS="-I/usr/local/ssl/include -I/usr/lib64 -I/usr/local/BerkeleyDB.4.7/include/"

WORKDIR /bitcoin-0.3.24/src
RUN strip bitcoind

FROM registry.suse.com/bci/bci-minimal:15.7
COPY --from=builder /bitcoin-0.3.24/src/bitcoind /usr/local/bin
