...
#bitcoin v0.9.3
WORKDIR /
RUN wget https://github.com/bitcoin/bitcoin/archive/refs/tags/v0.9.3.zip && \
    unzip v0.9.3.zip
WORKDIR /bitcoin-0.9.3
RUN sed -i '1i#include <stdarg.h>' /bitcoin-0.9.3/src/leveldb/util/posix_logger.h
RUN sed -i 's/va_copy/__va_copy/' /bitcoin-0.9.3/src/leveldb/util/posix_logger.h
COPY patch_mocacinno_chainparams /bitcoin-0.9.3
RUN patch /bitcoin-0.9.3/src/chainparams.cpp < patch_mocacinno_chainparams
RUN ./autogen.sh
RUN ldconfig
RUN ./configure  --with-cli --with-daemon CXX="g++ -std=c++11"

RUN make -j "$(($(nproc) + 1))"
WORKDIR /bitcoin-0.9.3/src
RUN strip bitcoind && strip bitcoin-cli

FROM registry.suse.com/bci/bci-minimal:15.7
COPY --from=builder /bitcoin-0.9.3/src/bitcoin-cli /usr/local/bin
COPY --from=builder /bitcoin-0.9.3/src/bitcoind /usr/local/bin
