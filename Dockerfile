...
RUN git checkout tags/v0.17.2 -b v0.17.2 #v0.17.2
RUN ./contrib/install_db4.sh `pwd` #v0.17.2
RUN zypper ref -s && zypper --non-interactive install libopenssl-devel
ENV BDB_PREFIX='/bitcoin/db4'
RUN ./autogen.sh #v0.17.2


RUN ./configure BDB_LIBS="-L${BDB_PREFIX}/lib -ldb_cxx-4.8" BDB_CFLAGS="-I${BDB_PREFIX}/include"  --enable-util-cli --enable-util-tx --enable-util-wallet --enable-util-util #v0.17.2
#RUN./configure BDB_LIBS="-L${BDB_PREFIX}/lib -ldb_cxx-4.8" BDB_CFLAGS="-I${BDB_PREFIX}/include" --enable-util-cli --enable-util-tx --enable-util-wallet --enable-util-util LDFLAGS="-L/boost_1_66_0/stage/lib" LIBS="-lboost_system -lboost_filesystem" #v0.17.2
RUN make -j "$(($(nproc) + 1))" #v0.17.2
WORKDIR /bitcoin/src
