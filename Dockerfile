...
#bitcoin v0.10.2
WORKDIR /
RUN wget https://github.com/bitcoin/bitcoin/archive/refs/tags/v0.10.2.zip && \
    unzip v0.10.2.zip
WORKDIR /bitcoin-0.10.2
RUN ./autogen.sh
RUN ldconfig
RUN ./configure  --enable-util-cli --enable-util-tx --enable-util-wallet --enable-util-util CXX="g++ -std=c++98"
RUN make -j "$(($(nproc) + 1))"
WORKDIR /bitcoin-0.10.2/src
RUN strip bitcoind && strip bitcoin-cli && strip bitcoin-tx

FROM registry.suse.com/bci/bci-minimal:15.7
COPY --from=builder /bitcoin-0.10.2/src/bitcoin-cli /usr/local/bin
COPY --from=builder /bitcoin-0.10.2/src/bitcoin-tx /usr/local/bin
COPY --from=builder /bitcoin-0.10.2/src/bitcoind /usr/local/bin
